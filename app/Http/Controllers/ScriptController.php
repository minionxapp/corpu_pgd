...
class ScriptController extends Controller
{
    ...
    public function scriptView($param)
    {
        ...
        $col = '';
        foreach (($objParam)->colum as $kolom) {
            $col = $col . "&lt;th&gt; " . $kolom->column . "&lt;/th&gt;\n";
        }
        $script = $script . $col;
        ...
        aria-labelledby=\"addDataLabel\" aria-hidden=\"true\"&gt;
            ...
            &lt;div class=\"modal-content\"&gt;
                ...
                &lt;div class=\"modal-body\"&gt;
                    &lt;form action=\"/add".$table."\" method=\"POST\" autocomplete=\"off\"&gt;
                        {{ csrf_field() }}\n";
        ...
        $col2 = '';
        foreach (($objParam)->colum as $kolom) {
            //cek unutk tipenya apakah Date atau bukan, lanjutkan.....
            $col2 = $col2 . "&lt;div class=\"form-group\"&gt;
            &lt;label for=\"".$kolom->column."\"&gt;".$kolom->column."&lt;/label&gt
            &lt;input type=\"text\" name=\"" . $kolom->column . "\" class=\"form-control\" id=\"" . $kolom->column . "\"&gt;
            &lt;/div&gt;\n";
        }
        ...
        $col3 = '';
        foreach (($objParam)->colum as $kolom) {
            $col3 = $col3 . "{ data: '" . $kolom->column . "', name: '" . $kolom->column . "' },  \n";
        }
        ...
        foreach (($objParam)->colum as $kolom) {
            $add = $add . "$('#" . $kolom->column . "').attr('readonly', true); \n";
        }

        foreach (($objParam)->colum as $kolom) {
            $add = $add . "$('#" . $kolom->column . "').val(''); \n";
        }
       ...
       success:function(data) {";
        foreach (($objParam)->colum as $kolom) {
            $view = $view . "$('#" . $kolom->column . "').val(data." . $kolom->column . "); \n";
        }
        ...
        function readonly(params) {
          ";
        foreach (($objParam)->colum as $kolom) {
            $view = $view . "$('#" . $kolom->column . "').attr('readonly', params); ";
        }
        $view = $view . " }\n
          &lt;/script&gt;\n
      @stop";
                ...
                if(\$request-&gt;id == null ){
                    ...
                    foreach (($objParam)->colum as $kolom) {
                        if ($kolom->column == 'id') {
                            ...
                            $script = $script. "\$model-&gt;".$kolom->column." = \$request-&gt".$kolom->column.";\n";
                        }
                    }
